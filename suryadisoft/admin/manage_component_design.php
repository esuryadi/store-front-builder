...
if (isset($Action) && $Action == "Update")
	$query = "SELECT * FROM COMPONENT_DESIGN WHERE component_name = '$component_name' AND design_style = '$design_style'";
else
	$query = "SELECT * FROM COMPONENT_DESIGN WHERE component_name = '$component_name' ORDER BY design_style";
$query_result = mysql_query($query);

$field_list = mysql_list_fields(_ADMIN_DATABASE,"COMPONENT_DESIGN");
...
function editComponentDesign(component_name,design_style) {
	var url = "manage_component_design.php?Action=Update&component_name=" + component_name + "&design_style=" + design_style;
	open(url,"_self");
}

function deleteComponentDesign(component_name,design_style) {
	var url = "manage_component_design_result.php?Action=Delete&component_name=" + component_name + "&design_style=" + design_style;
	open(url,"_self");
}

function changeComponent(component_name) {
	var url = "manage_component_design.php?component_name=" + component_name;
	open(url,"_self");
}
</script>
</head>
...
Built-in Components</a><strong> &gt; <font color="00aeef">Component Design</font></strong></font> 
<p>
  <? if (isset($Action)) {?>
</p>
<form action="manage_component_design_result.php" method="post" name="ComponentDesignForm" id="ComponentDesignForm">
		<input type="hidden" name="Action" value="<?=$Action?>">
		<table cellpadding="5" cellspacing="5">
			<? if ($Action == "Update") {
				$rs = mysql_fetch_row($query_result);?>
				<input type="hidden" name="old_design_style" value="<?=$rs[1]?>">
			<? }?>			 
      ...
      <td align="right" nowrap><font size="-1" face="Verdana, Arial, Helvetica, sans-serif">
		...
		<p>&nbsp;</p>			
  <p> 			
    <input type="submit" name="Submit" value="<?=$Action?> Component Design">
...
<center>
  <table border="0" cellpadding="0" cellspacing="0">
    <tr>
      <td><a href="manage_component_design.php?Action=Add&component_name=<?=$component_name?>"><img src="../images/new_component_design.gif" alt="New Component Design" width="154" height="21" border="0"></a></td>
    	<td>&nbsp;</td>
			<td><a href="manage_component_properties.php"><img src="../images/component_properties.gif" alt="Component Properties" width="137" height="21" border="0"></a></td>
		</tr>
  </table>
...
<tr>
	...
	<tr>
		...
		<? }?>
		<td width="43"><input name="Update" type="button" id="Update" value="Edit" onClick="editComponentDesign('<?=$rs[0]?>','<?=$rs[1]?>');"></td>
		<td width="288"><input name="Delete" type="button" id="Delete" value="Delete" onClick="deleteComponentDesign('<?=$rs[0]?>','<?=$rs[1]?>');"></td>
	</tr>
	<? }?>
</table>
<p>
